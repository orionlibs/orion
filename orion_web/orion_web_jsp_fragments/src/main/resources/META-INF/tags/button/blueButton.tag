    ...
    <c:when test="${hidden != null}">            
        <c:choose>
            <c:when test="${isSolid != null && isSolid}">            
                <button id="${id}" class="${classes} btn btn-primary" hidden="${hidden}">${text}</button>
            </c:when>
            <c:otherwise>        
                <button id="${id}" class="${classes} btn btn-outline-primary" hidden="${hidden}">${text}</button>
            </c:otherwise>
        </c:choose>
    </c:when>
    <c:otherwise>        
        <c:choose>
            <c:when test="${isSolid != null && isSolid}">            
                <button id="${id}" class="${classes} btn btn-primary">${text}</button>
            </c:when>
            <c:otherwise>        
                <button id="${id}" class="${classes} btn btn-outline-primary">${text}</button>
            </c:otherwise>
        </c:choose>
    </c:otherwise>
</c:choose>