    ...
    <c:when test="${hidden != null}">            
        <c:choose>
            <c:when test="${isSolid != null && isSolid}">            
                <button id="${id}" class="${classes} btn btn-warning" hidden="${hidden}">${text}</button>
            </c:when>
            <c:otherwise>        
                <button id="${id}" class="${classes} btn btn-outline-warning" hidden="${hidden}">${text}</button>
            </c:otherwise>
        </c:choose>
    </c:when>
    <c:otherwise>        
        <c:choose>
            <c:when test="${isSolid != null && isSolid}">            
                <button id="${id}" class="${classes} btn btn-warning">${text}</button>
            </c:when>
            <c:otherwise>        
                <button id="${id}" class="${classes} btn btn-outline-warning">${text}</button>
            </c:otherwise>
        </c:choose>
    </c:otherwise>
</c:choose>