    ...
    <c:when test="${hidden != null}">            
        <c:choose>
            <c:when test="${isSolid != null && isSolid}">            
                <button id="${id}" class="${classes} btn btn-danger" hidden="${hidden}">${text}</button>
            </c:when>
            <c:otherwise>        
                <button id="${id}" class="${classes} btn btn-outline-danger" hidden="${hidden}">${text}</button>
            </c:otherwise>
        </c:choose>
    </c:when>
    <c:otherwise>        
        <c:choose>
            <c:when test="${isSolid != null && isSolid}">            
                <button id="${id}" class="${classes} btn btn-danger">${text}</button>
            </c:when>
            <c:otherwise>        
                <button id="${id}" class="${classes} btn btn-outline-danger">${text}</button>
            </c:otherwise>
        </c:choose>
    </c:otherwise>
</c:choose>