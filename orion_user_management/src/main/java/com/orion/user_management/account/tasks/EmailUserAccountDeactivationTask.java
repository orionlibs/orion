...
public class EmailUserAccountDeactivationTask extends OrionService
{
    public static void run(String emailAddress)
    {
        ...
        String subject = ConfigurationService.getProp("user.management.account.disabled.email.subject");
        EmailData emailData = EmailService.prepareEmailFromTemplate(templateParameters,
                        "user.management.account.disabled.email.velocity.template",
                        "AccountDeactivationEmailTemplate",
                        subject,
                        emailAddress,
        ...
        {
            logError(UserAccountErrors.ErrorWithSendingAccountDeactivationEmail, e);
        }
