...
public class GetSumOfMultiSetsTask extends Orion
{
    @SuppressWarnings(
    {"unchecked", "rawtypes"})
    public static MultiSet run(MultiSet set1, OrionSet<ANumber> set2)
    {
        ...
        {
            result = ((OrionHashMultiSet<ANumber>)set1.getElements()).getSum((OrionHashMultiSet)set2);
        }
        else
        {
            result = ((OrionHashMultiSet<ANumber>)set1.getElements()).getSum(OrionHashMultiSet.of(set2));
        }
