...
public class GetVerticesOfGraphWithDistanceFromVertexGreaterThanOrEqualToGivenDistanceTask extends Orion
{
    public static Vertex[] run(Graph graph, ANumber distance, Vertex vertex)
    {
        ...
        Stream<Vertex> stream = graph.getVertices()
                        .filter(v -> v.notEquals(vertex) && graph.getDistanceBetween(vertex, v).isGreaterThanOrEqual(distance));
