...
public class GetHyperbolicSineInRadiansTask extends Orion
{
    public static ANumber run(ANumber x, int precision)
    {
        ...
        else
        {
            ...
            {
                return ANumber.of(Math.sinh(x.applyPrecisionGET(precision).get().doubleValue()));
            }
            else
            {
                ANumber eToTheX = Functions.exp.run(x);
                return eToTheX.subtractGET(eToTheX.reciprocateGET()).halfGET(precision);
            }
